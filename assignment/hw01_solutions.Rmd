...
>    b. Now explain in words and functional formula(e) how you might model the relationship between transit usage and density  as a linear regression model. Be sure to identify and justify your assignment of variables, and explain the assumptions of your population regression function.
...
>    d. Calculate the conditional means of Y for each value of X=x in the population. How are these results related to your population regression function?
...
>    e. Plot (R, SPSS, or another method) the following clearly:
>    
...
>    f. Explain the key features of your plot in e), keeping in mind this is a population regression! For one value of X, explain precisely how each of the Y|X values relates to the PRL. Feel free to sketch on a copy of the plot to make your points clear.
...
>	   g. Use equations to calculate and then interpret (making reference to your plot if useful)
...
>	     2). The total sum of squares (TSS)
...
R2 <- 1 - SSR/TSS
```
$\text{R}^2$ = `r R2`, which we can verify with R command `summary(lm(y ~ x))$r.squared`.

>	   h. Use the solved PRL to predict the value of $Y$ for an $X$ that does not appear in the data. Write out the equation and solution.
...
>	   i. Make a case for the inclusion of two additional explanatory/control variables you might include to better explain transit use (or density, if it is your dependent variable of choice). Describe your hypothesis of effect direction of the new variables, and how you would operationalize the variables (e.g. coding them as dummy variables).
...
>	  1). Interpret the significance of:
>	     - the model;
>	     - the estimate of $\beta_0$ and $\beta_1$.

The model is significant with p	value close to 0 and both intercept and the coefficient for BLDGSQFT are significant at 0.1% signficance level.
	  
>	  2). Construct 95% confidence intervals for each $\beta$ in your SRL estimates. Interpret the range and its statistical meaning.
...
>	  3). For question 1.i, in addition to describing your hypothesis of effect direction of the new variables, and how you would operationalize the variables (e.g. coding them as dummy variables), run regression models with the two additional variables and conduct hypothesis tests whether the full model with additional variables are statistically better than the restricted model you estimated earlier.
